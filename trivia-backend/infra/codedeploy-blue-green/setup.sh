...
aws cloudformation deploy --region us-east-1 --template-file packaged-template.yaml --stack-name TriviaBackendHooksTest --tags project=reinvent-trivia --capabilities CAPABILITY_NAMED_IAM --tags project=reinvent-trivia --parameter-overrides TriviaBackendDomain=api-test.awsuser.group

aws cloudformation deploy --region us-east-1 --template-file packaged-template.yaml --stack-name TriviaBackendHooksProd --tags project=reinvent-trivia --capabilities CAPABILITY_NAMED_IAM --tags project=reinvent-trivia --parameter-overrides TriviaBackendDomain=api.awsuser.group
